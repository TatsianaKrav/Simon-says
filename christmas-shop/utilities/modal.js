...
function renderModal() {
    const pathToImage = '../assets/images/gifts/gift-for-work.png';
    const giftName = 'gift';
    const giftPurpose = 'for work';
    const giftTitle = 'Console.log Guru';
    const giftDescription = 'Uses console.log like a crystal ball to find any issue.';
    ...
    const modalImage = createElement('div', 'gift-card-image');
    ...
    closeModal.innerHTML = `<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            `
    modalWindow.append(modalImage, closeModal);

    const img = createElement('img');
    modalImage.append(img);
    img.setAttribute('src', pathToImage);
    img.setAttribute('alt', giftName);

    const modalInfo = createElement('div', 'gift-card-info');
    modalWindow.appendChild(modalInfo);


    const giftPurposeEl = createElement('span', 'gift-purpose', giftPurpose);
    const giftTitleEl = createElement('h3', 'gift-card-title', giftTitle);
    const giftDescriptionText = createElement('p', 'gift-description', giftDescription);

    const giftSuperEl = createElement('div', 'gift-super');
    modalInfo.append(giftPurposeEl, giftTitleEl, giftDescriptionText, giftSuperEl);
    ...
    for (let i = 0; i < 4; i++) {
        const powerName = 'Live';
        const powerValue = '+500';
        const amount = powerValue.slice(1) / 100;

        const power = createElement('div', 'power');
        powerdsEl.appendChild(power);

        const powerTitle = createElement('span', 'power-title', powerName);
        ...
        const snowflake = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
